...
Shader "Enviro/EnviroSurfaceShader/Specular_Tesslation_Emission" {
    Properties {
        ...
        _EmissionStrenght ("Emission Strenght", Float ) = 0
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float node_9846 = (_DetailIntensity*_Mask_var.r);
                float3 node_498_nrm_base = _NormalMap_var.rgb + float3(0,0,1);
                float3 node_498_nrm_detail = lerp(_NormalClear_var.rgb,_DetailNormal_var.rgb,node_9846) * float3(-1,-1,1);
                ...
                float3 node_6140 = lerp((_Albedo_var.rgb*_AlbedoColor.rgb),_DetailAlbedo_var.rgb,node_9846);
                float3 diffuseColor = lerp(node_6140,lerp(node_6140,_SnowAlbedo_var.rgb,_SnowStrenght),_Mask_var.g);
                diffuseColor *= 1-specularMonochrome;
                float3 diffuse = (directDiffuse + indirectDiffuse) * diffuseColor;
                float4 _EmissionMap_var = tex2D(_EmissionMap,TRANSFORM_TEX(i.uv0, _EmissionMap));
                float3 emissive = (_EmissionStrenght*(_EmissionMap_var.rgb*_EmissionColor.rgb));
