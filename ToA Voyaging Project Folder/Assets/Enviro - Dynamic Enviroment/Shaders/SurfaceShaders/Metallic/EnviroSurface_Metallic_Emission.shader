...
Shader "Enviro/EnviroSurfaceShader/Metallic_Emission" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float node_7004 = (_DetailIntensity*_Mask_var.r);
                float3 node_498_nrm_base = _NormalMap_var.rgb + float3(0,0,1);
                float3 node_498_nrm_detail = lerp(_NormalClear_var.rgb,_DetailNormal_var.rgb,node_7004) * float3(-1,-1,1);
                ...
                float3 node_381 = lerp((_Albedo_var.rgb*_AlbedoColor.rgb),_DetailAlbedo_var.rgb,node_7004);
                float4 _SnowAlbedo_var = tex2D(_SnowAlbedo,TRANSFORM_TEX(i.uv0, _SnowAlbedo));
                float3 diffuseColor = lerp(node_381,lerp(node_381,_SnowAlbedo_var.rgb,_SnowStrenght),_Mask_var.g); 
