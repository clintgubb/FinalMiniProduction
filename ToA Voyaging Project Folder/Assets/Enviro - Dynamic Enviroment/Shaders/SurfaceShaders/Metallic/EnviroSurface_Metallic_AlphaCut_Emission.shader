...
Shader "Enviro/EnviroSurfaceShader/Metallic_AlphaCut_Emission" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3 node_6356 = lerp((_Albedo_var.rgb*_AlbedoColor.rgb),_DetailAlbedo_var.rgb,node_4161);
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3 node_6356 = lerp((_Albedo_var.rgb*_AlbedoColor.rgb),_DetailAlbedo_var.rgb,node_4161);
                float4 _SnowAlbedo_var = tex2D(_SnowAlbedo,TRANSFORM_TEX(i.uv0, _SnowAlbedo));
                float3 diffuseColor = lerp(node_6356,lerp(node_6356,_SnowAlbedo_var.rgb,_SnowStrenght),_Mask_var.g); // Need this for specular when using metallic
                float specularMonochrome;
                float3 specularColor;
                diffuseColor = DiffuseAndSpecularFromMetallic( diffuseColor, lerp(_Metallic_var.r,_SnowAlbedo_var.a,_SnowStrenght), specularColor, specularMonochrome );
                specularMonochrome = 1-specularMonochrome;
